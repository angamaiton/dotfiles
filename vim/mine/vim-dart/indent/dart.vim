...
" URL: https://github.com/pangloss/vim-dart
...
" vim  -eNs "+filetype plugin indent on" "+syntax on" "+set ft=dart" \
...
" au! Filetype dart let b:syng_str = '^\%(.*template\)\@!.*string\|special'
" au! Filetype dart let b:syng_strcom = '^\%(.*template\)\@!.*string\|comment\|regex\|special\|doc'
...
let s:opfirst = '^' . get(g:,'dart_opfirst',
      \ '\C\%([<>=,.?^%|/&]\|\([-:+]\)\1\@!\|\*\+\|!=\|in\%(stanceof\)\=\>\)')
let s:continuation = get(g:,'dart_continuation',
...
function GetDartIndent()
  ...
  elseif idx == -1 && s:LookingAt() == '(' && &cino =~ '(' &&
    ...
    if !pval
      ...
      if search('\m'.get(g:,'dart_indent_W_pat','\S'),'W',num)
