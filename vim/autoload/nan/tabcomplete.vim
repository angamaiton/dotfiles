...
function! s:NAN_Tab() abort
  " Advance and select autocomplete result
  if g:nan_tab_completion && pumvisible()
    return "\<C-n>"
  endif
...
function! s:NAN_STab() abort
  return g:nan_tab_completion && pumvisible() ? "\<C-p>" : "\<C-d>"
endfunction

function! nan#tabcomplete#Init() abort
  silent! iunmap <Tab>
  silent! iunmap <S-Tab>

  " Tab - requires noremap since it may return a regular <Tab>
  inoremap  <silent><special><expr>  <Tab>     <SID>NAN_Tab()
  imap      <silent><special><expr>  <S-Tab>   <SID>NAN_STab()
endfunction
